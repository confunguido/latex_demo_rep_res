...
colnames(ICER_QALY)= c("Both-High", "Low-Sensitivity", "Low-Specificity")
icer_data = cbind(data.frame(vaccine_cost = unit_cost_vaccine), ICER_QALY)
write_csv(x = icer_data, path = "../output/report_table_cost_effectiveness_QALY.csv")

if(save.fig == T){
  jpeg(sprintf('../figures/report_figure_ICER_%s_%s_10y.jpeg', perspective,country),
       width=5,height=5,units='in',res=300)
}
plot(unit_cost_vaccine, ICER_QALY[,1], type = "l", col = "black",
     main = "Cost-effectiveness", ylab = "ICER", xlab = "Vaccine (USD)",
     ylim = c(0,3*econ$gdp))
lines(unit_cost_vaccine, ICER_QALY[,2], type = "l", col = "red")
lines(unit_cost_vaccine, ICER_QALY[,3], type = "l", col = "blue")
abline(h = econ$gdp, col = "black")
abline(h = 3*econ$gdp, col = "black")
abline(h = 0, col = "black")
legend(x = 0, y = 3*econ$gdp * 0.9,
       legend = c(sprintf("sens:%.2f,spec:%.2f",screening_sen[1],screening_sp[1]),
                  sprintf("sens:%.2f,spec:%.2f",screening_sen[2],screening_sp[2]),
                  sprintf("sens:%.2f,spec:%.2f",screening_sen[3],screening_sp[3])),
